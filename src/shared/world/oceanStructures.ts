...
import { BIOME_DEEP_OCEAN, BIOME_BEACH, BIOME_WARM_OCEAN, isOceanBiome } from './biomeIds';
...
export const OCEAN_STRUCTURES: GridType[] = [
  {
    ...
    site: (gen, x, z, inf) => {
      ...
      if (inf.biome !== BIOME_DEEP_OCEAN) return null;
      for (const [dx, dz] of [[16, 0], [-16, 0], [0, 16], [0, -16]]) {
        if (gen.columnInfo(x + dx, z + dz, tmp).biome !== BIOME_DEEP_OCEAN) return null;
      }
  ...
  },
  {
    ...
    build: (c, s, gen) => buildOceanRuins(c, s, gen, gen.columnInfo(s.x, s.z, tmp).biome === BIOME_WARM_OCEAN),
  },
  {
    key: 'buried_treasure', spacing: TREASURE_SPACING, separation: 1, salt: 10387320, radius: 1,
    site: (gen, x, z, inf) => {
      if (inf.biome !== BIOME_BEACH || (hash2(x, z, gen.seed ^ 0x7e5a) % 1000) / 1000 >= TREASURE_CHANCE) return null;
      const h = gen.surfaceAt(x, z, gen.columnInfo(x, z, tmp));
