...
export class Interaction {
  ...
  releaseBow(dir: number[]): void {
    ...
    if (f < 0.1) return;
    const en = this.g.enchant.heldField(); // Fase 7 (encantamientos): Poder, Retroceso, Fuego e Infinidad
    if (!this.g.creative) {
      if (enchLevel(this.g.heldStack, INFINITY) > 0 ? this.g.inv.count(ARROW) < 1 : this.g.inv.remove(ARROW, 1) < 1) return;
      this.wearHeld(1);
    }
    const p = this.g.player;
    this.g.net?.send({ t: 'shoot', p: [p.x + dir[0] * 0.3, p.eyeY - 0.1, p.z + dir[2] * 0.3], d: [dir[0], dir[1], dir[2]], f, ...en });
    this.g.swing(false);
  }
  ...
  onInteractResult(msg: Extract<ServerMsg, { t: 'ires' }>): void {
    ...
    if (msg.give && isValidItem(msg.give.id)) {
      const give = { id: msg.give.id, count: Math.max(1, msg.give.count | 0), ...(msg.give.dmg ? { dmg: msg.give.dmg } : {}), ...(msg.give.data ? { data: msg.give.data } : {}) }; // Fase 6.5: con su desgaste y sus datos
